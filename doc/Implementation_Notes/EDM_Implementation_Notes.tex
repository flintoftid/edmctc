...
noticeable. The fitting method of determining fittingMFP = 4S/fittingS does not appear to be
...
\begin{align}
\begin{matrix}
\uvn\vcdot\left[D\pr\vnabla w\prt\right] + h\pr w\prt + J_\mathrm{out}\prt - J_\mathrm{in}\prt = 0 &\vr\in\partial\Omega
\end{matrix}
\end{align}
where $\uvn$ is an outward normal vector to the domain and 
$h\pr$ is an energy exchange coefficient (EC). $J_\mathrm{out}\prt$ and
$J_\mathrm{in}\prt$ represent an in-flux and out-flux (exitance) of energy density respectively and
can be used to model sources and energy exchange boundary conditions (EEBCs) are noted below.
...
\end{matrix}
\end{align}
where the exitance, $J_\rs\prt$, is
\begin{align}
...
boundary condition (FCBC) through the aperture:
\begin{align}
\begin{matrix}
...
\end{align}
where $a(w,u)$ is a bilinear form
\begin{align}
a(w,u)
=\iiint_{\Omega} D\pr{\vnabla}w\prt{\vcdot}{\vnabla}u\pr\dif{V}
+\Lambda_V \iiint_{\Omega} u\pr w\prt\dif{V}
...
The above development is sufficient to implement a numerical solution in FreeFEM++; for 
...
suitably constructed FE domain or using a macro, for example
\begin{verbatim}
...
\subsection[Discretisation of the diffusion problem]{Discretisation of the diffusion problem}
...
\end{align}
The elements are typically triangles in 2D or tetrahedra in 3D.
...
\end{align}
where $X$ denoted the types of finite elements. For example piecewise continuous elements 
are denoted $P_1$. Here $M$ is the dimension of $V_h$, i.e. the total number of vertices, 
...
Now using the Galerkin method we approximate 
\begin{align}
...
= 0.
\end{align}
Denoting the discrete operators
\begin{align}
...
= 0,
\end{align}
where $D_{ij}$ is the stiffness matrix, $M_{ij}$ is the mass
matrix and $\Lambda_V M_{ij}+H_{ij}$ is the dissipation matrix. This can be written in matrix form as
\begin{align}
\vec{u}^\rT \mM \dot{\vec{w}} + \vec{u}^\rT\mD \vec{w} +\Lambda_V \vec{u}^\rT\mM \vec{w}+ \vec{u}^\rT\mH \vec{w}
-\vec{u}^\rT\vec{J}^\rs -\vec{u}^\rT\vec{P} = 0.
\end{align}
...
\end{align}
for time-independent problems. Time dependent systems can be solved by applied a finite difference
scheme to the time derivative.
...
\end{matrix}
\end{align}
The system can be solved using a Robin-Robin iterative algorithm. We first initialise
...
\end{align}
in terms of the net energy flux $J_{21}$ from domain 1 into domain 2. At each iteration, $n$, this can be
...
Continuity of both the energy density and flux can be enforced using a Schwarz domain decomposition
method. The weak forms are the same as for the FCBC method, however the net flux is updated using
\begin{align}
J^0_{21}\prt &= 0 \\
...
is equivalent to using a single domain method.
