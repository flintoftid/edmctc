...
\end{align}
as is the corresponding overall diffusivity
\begin{align}
...
can be used to model sources and energy exchange boundary conditions (EEBCs) as noted below.
...
A modified form based on the Eyring reverberation time has also been applied:
\begin{align}
\begin{matrix}
...
results over the full range of absorption efficiency:
\begin{align}
\begin{matrix}
...
be subtract to give the reverberant energy density; in 3D this correction is:
\begin{align}
...
\end{align}
and the reverberant energy density flux is
\begin{align}
...
=\iiint_{\Omega} u\pr p\prt\dif{V}.
\end{align}
The most general form of the Robin boundary condition
\begin{align}
...
\end{align}
where $a(w,u)$ is the bilinear form
\begin{align}
a(w,u)
=\iiint_{\Omega} D\pr{\vnabla}w\prt{\vcdot}{\vnabla}u\pr\dif{V}
+\Lambda_V \iiint_{\Omega} u\pr w\prt\dif{V}
...
\end{align}
where $vec{u}$ and $\vec{w}$ of vectors containing the DOFs and ${}^\rT$ denoted the transpose. 
...
\end{align}
for time-independent problems. Time dependent systems can be solved by applying a finite difference
