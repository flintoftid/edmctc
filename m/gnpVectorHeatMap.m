function gnpVectorHeatMap( x , y, Vx , Vy , xMin , xMax , xLabel , yMin , yMax , yLabel , vMin , vMax , vLabel , tag )
  ...
  fprintf( fp , 'splot "%s" using 1:2:(db10($3)) ti "" with pm3d , "%s" every %d:%d using ($1-xf($4)):($2-yf($4)):(0):(2*xf($4)):(2*yf($4)):(0.0) ti "" with vectors head size 0.05,10,30 filled lc "black"\n' , datFileName , datFileName , stepX , stepY );
  fclose( fp );
